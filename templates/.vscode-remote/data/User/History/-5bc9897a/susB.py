...
from view_menu.froms import MenuForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if menu_type == 'BREAKFAST':
                ...
                breakfast_form = menu_details
                breakfast = breakfast_form.save()
                # Additional processing if needed
                
            elif menu_type == 'LUNCH':
                # Save data to Lunch model
                lunch_form = menu_details
                lunch = lunch_form.save()
                # Additional processing if needed
                
            elif menu_type == 'DINNER':
                # Save data to Dinner model
                dinner_form = menu_details
                dinner = dinner_form.save()
                # Additional processing if needed
                
            elif menu_type == 'SIDE':
                # Save data to Sides model
                sides_form = menu_details
                sides = sides_form.save()
                # Additional processing if needed
