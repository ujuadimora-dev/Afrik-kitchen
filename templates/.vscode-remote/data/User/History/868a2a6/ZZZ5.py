...
class BookingView(FormView):
    ...
    def form_valid(self, form):
        ...
        if avaliable_tables:
            ...
            # Get the reservation time label from the BOOKING_TIME choices
            reservation_time_label = dict(TableAvaliableForm.BOOKING_TIME).get(int(data['reservation_time']), '')

            # Format the reservation date and pass it to the template
            reservation_date_formatted = data['reservation_date'].strftime('%B %d, %Y')

            # Pass the customer name, reservation time, and other details to the success template
            context = {
                'customer_name': customer_name,
                'capacity': data['capacity'],
                'table_number': table.table_num,
                'reservation_date': reservation_date_formatted,
                'reservation_time_label': reservation_time_label,
                'table_position': table.table_position,
